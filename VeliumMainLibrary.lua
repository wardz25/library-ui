...
function VoidUI:divider(parent, lo)
	local d = self:frame(parent, UDim2.new(1, 0, 0, 1), nil, Color3.fromRGB(30, 15, 55))
	d.LayoutOrder = lo
	return d
end
...
function VoidUI:accordion(parent, title, lo, startOpen)
	local T = self.T
	local header = self:frame(parent, UDim2.new(1, 0, 0, 32), nil, Color3.fromRGB(7, 4, 14))
	...
	hitBtn.ZIndex = 5
	local body = self:frame(parent, UDim2.new(1, 0, 0, 0), nil, Color3.fromRGB(7, 4, 14))
	body.LayoutOrder = lo + 1
	body.AutomaticSize = Enum.AutomaticSize.Y
	body.Visible = startOpen ~= false
	self:corner(body, 6)
	self:stroke(body, T.ACCENT, 1)
	local inner = self:frame(body, UDim2.new(1, 0, 1, 0), nil, Color3.fromRGB(7, 4, 14), 0)
...
function VoidUI:inlinePickerDropdown(rowParent, overlayParent, config)
	...
	local ohdr = self:frame(overlay, UDim2.new(1, 0, 0, 24), nil, Color3.fromRGB(9, 6, 16))
	self:corner(ohdr, 6)
...
function VoidUI:iconBtn(parent, icon, label)
	...
	b.MouseEnter:Connect(function()
		if not accentBar.Visible then
			...
			iconLbl.TextColor3 = Color3.fromRGB(195, 155, 255)
			textLbl.TextColor3 = Color3.fromRGB(195, 155, 255)
		end
	end)
	...
	local function setActive(s)
		accentBar.Visible = s
		if s then
			b.BackgroundColor3 = Color3.fromRGB(55, 25, 100)
...
function VoidUI:sidebarDivider(parent)
	...
	d.BackgroundColor3 = Color3.fromRGB(30, 15, 55)
	d.BorderSizePixel = 0
	return d
end
...
function VoidUI:builtinTeamCard(parent, name, desc, lo, onEquip)
	local T = self.T
	local card = self:frame(parent, UDim2.new(1, 0, 0, 52), nil, Color3.fromRGB(9, 6, 16))
	card.LayoutOrder = lo
	self:corner(card, 6)
	...
	local badge = self:frame(card, UDim2.new(0, 74, 0, 14), UDim2.new(0, 8, 0, 4), Color3.fromRGB(55, 25, 100))
	self:corner(badge, 4)
	...
	local badgeLbl = self:label(badge, "BUILT-IN", UDim2.new(1, -14, 1, 0), UDim2.new(0, 13, 0, 0), Color3.fromRGB(195, 155, 255), 7, Enum.TextXAlignment.Center)
	badgeLbl.Font = Enum.Font.GothamBold

	
	local nameLbl = self:label(card, name, UDim2.new(1, -100, 0, 16), UDim2.new(0, 8, 0, 18), Color3.fromRGB(195, 155, 255), 10)
	nameLbl.Font = Enum.Font.GothamBold
	local descLbl = self:label(card, desc, UDim2.new(1, -100, 0, 13), UDim2.new(0, 8, 0, 35), T.DIM, 8)
	descLbl.Font = Enum.Font.Gotham

	
	local veliumLogo = Instance.new("ImageLabel", card)
	...
	local equipBtn = self:button(card, "⇄", UDim2.new(0, 28, 0, 28), UDim2.new(1, -36, 0.5, -14), Color3.fromRGB(55, 25, 100), Color3.fromRGB(195, 155, 255), 12)
	self:stroke(equipBtn, Color3.fromRGB(100, 60, 180), 1)
...
function VoidUI:teamCard(parent, name, petNames, count, lo, onEquip, onDelete)
	...
	subContainer.ScrollBarImageColor3 = Color3.fromRGB(155, 100, 255)
	...
	for i, entry in ipairs(summaryParts) do
		...
		if mutStr then
			...
			lblMut.TextSize = 8
			lblMut.Text = "[" .. mutStr .. "]"
			lblMut.TextColor3 = Color3.fromRGB(190, 110, 255)
...
function VoidUI:timingEditor(acInner, pageFrame, CFG, D, saveD)
	...
	local teBtn = self:button(
		...
		UDim2.new(1, 0, 0, 24),
		nil,
		Color3.fromRGB(14, 10, 22),
		Color3.fromRGB(195, 155, 255),
		9
	)
	self:stroke(teBtn, Color3.fromRGB(100, 60, 180), 1)
	teBtn.TextXAlignment = Enum.TextXAlignment.Left
	self:pad(teBtn, 0, 8, 8, 0)
	local teBtnArrow = self:label(teBtn, ">", UDim2.new(0, 16, 1, 0), UDim2.new(1, -20, 0, 0), Color3.fromRGB(120, 75, 210), 11, Enum.TextXAlignment.Center)
	...
	local function secHdr(label, lo)
		...
		f.LayoutOrder = lo
		local l = self:label(f, label, UDim2.new(1, 0, 1, 0), nil, Color3.fromRGB(90, 60, 150), 8)
		l.Font = Enum.Font.GothamBold
		return f
	end
	...
	local function kvRow(label, badge, badgeColor, dataKey, lo)
		...
		self:corner(bdg, 3)
		local bdgLbl = self:label(bdg, badge, UDim2.new(1, 0, 1, 0), nil, Color3.fromRGB(225, 215, 255), 7, Enum.TextXAlignment.Center)
		...
		local secLbl = self:label(row, " sec", UDim2.new(0, 24, 1, 0), UDim2.new(1, -24, 0, 0), Color3.fromRGB(80, 55, 130), 8, Enum.TextXAlignment.Left)
	...
	local function totalRow(label, id, lo, accent)
		local row = self:frame(inner, UDim2.new(1, 0, 0, 22), nil, Color3.fromRGB(9, 6, 16))
		row.LayoutOrder = lo
		self:corner(row, 4)
		self:stroke(row, accent and T.ACCENT or T.STROKE, 1)
		local lbl = self:label(row, label, UDim2.new(1, -60, 1, 0), UDim2.new(0, 8, 0, 0), accent and Color3.fromRGB(200, 175, 255) or T.DIM, 8)
		lbl.Font = Enum.Font.Gotham
		local val = self:label(row, "–", UDim2.new(0, 52, 1, 0), UDim2.new(1, -56, 0, 0), accent and Color3.fromRGB(210, 175, 255) or T.ACCENT, accent and 10 or 9, Enum.TextXAlignment.Right)
	...
	local COLOR_ALL  = Color3.fromRGB(38, 22, 70)
	local COLOR_KOI  = Color3.fromRGB(20, 10, 55)
...
function VoidUI:modePickerRow(parent, config)
	...
	local overlay = self:frame(overlayParent, UDim2.new(1,0,0,0), UDim2.new(0,0,0,0), Color3.fromRGB(7, 4, 14))
	...
	local innerList = self:frame(ovScroll, UDim2.new(1,0,0,0), nil, Color3.fromRGB(7, 4, 14), 0)
	...
	local function refreshCards()
		for _, ref in ipairs(cardRefs) do
			...
			ref.card.BackgroundColor3   = isSel and Color3.fromRGB(55, 25, 100) or Color3.fromRGB(9, 6, 16)
			local s = ref.card:FindFirstChildOfClass("UIStroke")
			if s then s.Color = isSel and T.ACCENT or T.STROKE end
			ref.badge.BackgroundColor3  = isSel and T.ACCENT or Color3.fromRGB(55, 30, 100)
	...
	for i, mode in ipairs(modes) do
		...
		local card = self:frame(innerList, UDim2.new(1,0,0,46), nil,
			isSel and Color3.fromRGB(55, 25, 100) or Color3.fromRGB(9, 6, 16))
		card.LayoutOrder = i
		card.ZIndex = 41
		self:corner(card, 5)
		...
		local badge = self:frame(card, UDim2.new(0,22,0,22), UDim2.new(0,6,0.5,-11),
			isSel and T.ACCENT or Color3.fromRGB(55, 30, 100))
		badge.ZIndex = 42
		self:corner(badge, 4)
